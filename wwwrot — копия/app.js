document.addEventListener('DOMContentLoaded', function() {
  ...
  document.addEventListener('DOMContentLoaded', function() { // Убедитесь, что DOM загружен
    const clearButton = document.getElementById('clear-search');
    const searchInput = document.getElementById('search-input');
  
    if (clearButton && searchInput) { // Проверка, что элементы существуют
      clearButton.addEventListener('click', function() {
        searchInput.value = '';
      });
    } else {
      console.warn('Кнопка очистки или поле поиска не найдены!');
    }
  });